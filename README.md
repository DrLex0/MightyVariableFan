...
This has so far only been tested with *PrusaSlicer* (formerly Slic3r) and (albeit not thoroughly) *Simplify3D.* Quite likely the post-processing script that generates the beep sequences will at this moment only work with G-code generated by those slicers. Of course the main reason why I have published this on *GitHub* is to make it easy for anyone to modify the code and submit pull requests to make it work with other slicer programs.

I have been using this for a year without problems, hence I have created a 1.0 release. There are some minor problems left and some things I would like to improve upon, but I consider the 1.0 release (and newer) very usable. See the *Current Issues* section at the bottom for caveats.<br>
...
Recently a pull request has been proposed for Sailfish to make the software PWM respond to speed arguments in the G-code, but this still has problems: the commands are still executed with horribly bad timing and there is no manual override (even if it would be added, manipulating it mid-print via the LCD menu is very kludgy). Also, GPX needs to be patched and recompiled to output X3G files with fan speed values included, and this will only work if you have somehow managed to have your slicer generate G-code that is suitable for Sailfish *and* includes variable fan speeds.
...
* If you already have installed a Raspberry Pi in your printer to have a camera feed or control other parts of the printer, you only need to install a USB sound card with microphone, an extra MOSFET, and some wires. (See the PAQ below though.)
...
Yes it is. I have been using it for more than a year now and despite the few glitches mentioned in the ‘current issues’, it works incredibly well.
...
* A tiny **microphone** that can be mounted very close (within 10 mm) to the buzzer. I recommend to build your own microphone with a standard electret capsule (9.7mm diameter), a bit of shielded cable, and a 3.5mm plug. This can be mounted perfectly onto the buzzer with a 3D printed part.
* A **24V MOSFET** break-out board. There is a very common one for the IRF520, which is very easy to mount inside the printer with yet another printed part.
...
* A **fan** that supports PWM. If your FFCP came with the ‘squirrel cage’ fan on the left of the carriage, you should be fine. If you don't have a fan yet, it would be a good idea to buy that exact same fan as FlashForge uses, which in my printer is a *Ruiapple DB5015HB24.* If you want to buy another type of fan, you should ensure it can be driven with a PWM signal, which is often *not* the case. Safest is to obtain a 4-wire fan (typically used for PC coolers) that has a separate PWM input, in that case you don't even need the MOSFET. The only problem is that you may have a hard time finding such a fan that works on 24 V, most will require 12 V. You can of course avoid this problem by installing an extra 12 V power supply or step-down converter.
...
Next, you need some **3D printed parts** to mount the components. They can be found in the *models* folder. The recommended layer height for all these is 0.2 mm.

* **A mount for the Raspberry Pi.** If you mount it in the position I recommend below, the `RaspberryPiMount` brackets should be optimal. These were designed for a model 3B but should in theory fit any model. You need two 2.2×6.5 mm self-tapping screws to secure the Pi after sliding it into the rear notches. To ensure the underside of the mount is flat and easy to glue, rub it against a sheet of sandpaper with the Pi mounted. The regular bracket raises the Pi **23 mm** from the bottom, which is a bit of a compromise between offering room for wires plugged into the GPIO pins, and avoiding that the microUSB power cable needs to bend too much due to the cramped space. Use this one if the power cable is not very flexible. You will need low-profile GPIO header plugs or angled plugs, the regular tall plugs won't fit. The `19mm` brackets raise the Pi only **19 mm,** which offers more room for GPIO plugs but will require a small microUSB power plug with a flexible cable. Of course you can also design your own brackets or use any other mount that works.<br>
  ![Pi mount](images/pi_mount1_small.jpg)[🔎](images/pi_mount1.jpg)
* **A mount for the MOSFET.** If you use the ubiquitous IRF520 break-out board, the `IRF520-MosfetMount.stl` model should be suitable, it uses the same 2.2 mm self-tapping screws as the mount for the Pi.<br>
  ![MOSFET](images/mosfet_small.jpg)[🔎](images/mosfet.jpg)
* **An adaptor to mount the microphone** onto the buzzer of the printer's main board. Two models are provided that fit a standard 9.7 mm electret capsule. As described above, if you want to keep the buzzer sounds as they are, use the half-open model `BUZZ-Electret-mount.stl`. The closed model `BUZZ-electret-mount-sealed.stl` can be used to mute most of the sound of the buzzer in case you don't want to hear the beep sequences and don't care that other beeps like the start-up song will be muted as well. It is strongly recommended to print this adaptor in a *flexible* filament to dampen vibrations and make mounting easier. If you are using a different model of microphone, again you will need to figure out something on your own, but make sure the mic is as close as possible to the buzzer and there is minimal contact with anything that vibrates.<br>
  ![Microphone 1](images/mic1_small.jpg)[🔎](images/mic1.jpg)


### Step 4: sanity check

Before mounting everything, it is a good idea to do a sanity check of the whole system outside the printer. The nice thing about the typical IRF520 board is that it has an LED that will light up if there is a signal on its GPIO input, even if nothing else is connected. Plug the sound card into the Pi and connect the MOSFET to the GPIO. The GPIO pins #12 (GPIO 18) and #14 (ground) must be connected respectively to the ‘SIG’ and ‘GND’ contacts of the MOSFET board ([here](https://www.raspberrypi.org/documentation/usage/gpio/) is documentation about the GPIO pins).
...
Once you have the setup running, all that is left to be done is to end up with print files that contain beep sequences instead of the old M126 and M127 commands. To do this, your regular G-code must be run through the *pwm_postprocessor.py* script. This script has so far only been tested with G-code produced by *PrusaSlicer* and *Simplify3D.* It might work with other slicers, but most likely the script will require a few modifications. (If you can make those modifications yourself, a pull request to merge the improvements would be welcome. If not, you can also send me a few G-code files and I can see how to make the script work for them.)

The problem is that some slicer programs are aware of the limited fan capability of MightyBoard-based printers, hence they only use fan on/off commands without any speed information if you ask them to output Sailfish-compatible G-code. There are two possible situations:
1. Your slicer outputs M126 commands without an ‘`S`’ argument. This is the case with PrusaSlicer or Slic3r. In this case, the solution is to change your printer profile to output G-code for *RepRap* instead, which contains M106 and M107 commands with speed arguments.
2. Your slicer does output M126 commands with an `S` argument. This is the case for Simplify3D. In this case, you must invoke the postprocessing script with an extra `-S` argument to tell it to accept the M126 commands.

The `-S` option can also be used to convert files that contain M126 and M127 commands without speed arguments. In that case the speed for M126 commands will be assumed to be maximum.

You need *Python 3.5* or newer on the machine where you'll be running the postprocessor script. Inside the script, you may need to make one important change: ensure `END_MARKER` is set to a line that indicates the print has ended. If you are using your own snippet of end G-code, just ensure it starts with a unique comment line and copy that exact line into the script. Of course you can avoid having to modify the script by just inserting the default `END_MARKER` line into your own end G-code.

There are other adjustments you can make, these can also be passed as command-line arguments:
...
Once the script has been configured, you can either manually run it on every G-code file you want to print (run with `-h` for more information), or you can somehow automate it inside your workflow. How you can do the latter, depends on your slicing program. In case you use PrusaSlicer with [my (DrLex) configuration](https://www.dr-lex.be/software/ffcp-slic3r-profiles.html), the latest version of the `make_fcp_x3g` script can invoke the PWM post-processing script as well.

**Note:** even if you reconfigure your slicer to output RepRap-flavor G-code, the GPX program to convert G-code into X3G must still be configured to output code for your specific printer model (e.g. `-m fcp` for the FlashForge Creator Pro)!
