...
Once the microphone gain has been tuned, restart calibration mode with `beepdetect -c` and play the first part of the BeepCalibration file again, then press ctrl-C and look at the output. The script will give:
* **a chunks-per-second measure:** this was already covered in the performance check above. This is only accurate if the script has been running for at least a few dozen seconds.
