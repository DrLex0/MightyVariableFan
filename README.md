...
This has so far only been tested with *Slic3r* and (albeit not thoroughly) *Simplify3D.* Quite likely the post-processing script that generates the beep sequences will at this moment only work with G-code generated by those slicers. Of course the main reason why I have published this on *GitHub* is to make it easy for anyone to modify the code and submit pull requests to make it work with other slicer programs.
...
Once you have the setup running, all that is left to be done is to end up with print files that contain beep sequences instead of the old M126 and M127 commands. To do this, your regular G-code must be run through the *pwm_postprocessor.py* script. This script has so far only been tested with G-code produced by *Slic3r* and *Simplify3D.* It might work with other slicers, but most likely the script will require a few modifications. (If you can make those modifications yourself, a pull request to merge the improvements would be welcome. If not, you can also send me a few G-code files and I can see how to make the script work for them.)
...
1. Your slicer outputs M126 commands without an ‘`S`’ argument. This is the case with Slic3r. In this case, the solution is to change your printer profile to output G-code for *RepRap* instead, which contains M106 and M107 commands with speed arguments.
